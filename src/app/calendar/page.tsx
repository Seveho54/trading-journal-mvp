...
export default function CalendarPage() {
  ...
  const worst = sortedAllDays.length
    ? sortedAllDays[sortedAllDays.length - 1]
    : null;
  ...
  const maxAbsPnl = useMemo(() => {
    ...
    return Math.max(
      1,
      ...byDay.map((d: any) => Math.abs(d.totalNetProfit ?? 0)),
    );
  }, [byDay]);

  useEffect(() => {
    track("view_calendar_page");
  }, []);
  ...
  const monthStats = useMemo(() => {
    const monthDays = byDay.filter((d: any) =>
      String(d.day ?? "").startsWith(currentMonthKey),
    );
    const tradingDays = monthDays.length;

    const monthNetPnl = monthDays.reduce(
    ...
    const wins = positionsInMonth.filter(
    ...
    const monthTrades = trades.filter((t: any) => {
      ...
      const statusOk =
        !t.status || String(t.status).toUpperCase() === "EXECUTED";
      ...
      <div className="card" style={{ padding: 14, marginBottom: 14 }}>
        ...
        >
          ...
          >
            ...
            <div style={{ marginTop: 6 }}>
              Net Profit:{" "}
              <span
                className={pnlClass(best.totalNetProfit)}
          ...
          >
            ...
            <div style={{ marginTop: 6 }}>
              Net Profit:{" "}
              <span
                className={pnlClass(worst.totalNetProfit)}
                style={{ fontWeight: 900 }}
              >
        ...
        >
          ...
          <div style={{ marginLeft: "auto", fontSize: 12 }}>
            Scale max day |PnL|:{" "}
            <b style={{ color: "var(--text)" }}>{fmtMoney(maxAbsPnl, ccy)}</b>
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 14, marginBottom: 12 }}>
        ...
        >
          ...
          <div>
            ...
            <div
              className={pnlClass(monthStats.monthNetPnl)}
          ...
          <div>
            ...
            <div
              className={pnlClass(monthStats.avgDailyPnl)}
              style={{ fontSize: 20, fontWeight: 900 }}
            >
      ...
      <div className="card" style={{ padding: 14, marginBottom: 14 }}>
        ...
        >
          {["Mo", "Di", "Mi", "Do", "Fr", "Sa", "So"].map((w) => (
            <div
              key={w}
              style={{ fontWeight: 800, opacity: 0.8, fontSize: 12 }}
            >
              {w}
            </div>
          ))}
          ...
          {days.map((d) => {
            ...
            return (
              <div
                key={key}
                onClick={() =>
                  clickable && router.push(`/positions?day=${key}`)
                }
                style={{
                  border: "1px solid var(--border)",
                  borderRadius: 12,
                  ...
                  background: cell
                    ? heatBg(cell.totalNetProfit ?? 0, maxAbsPnl)
              ...
              >
                ...
                >
                  ...
                  {clickable ? (
                    <div style={{ fontSize: 11, opacity: 0.75 }}>→</div>
                  ) : null}
                </div>
                  ...
                  {cell ? (
                    <>
                      <div>
                        PnL:{" "}
                        <span
                          className={pnlClass(cell.totalNetProfit ?? 0)}
                          style={{ fontWeight: 900 }}
                        >
          ...
          <>
            ...
            <div style={{ marginTop: 10, overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr>
                    {["Day", "Positions", "Net Profit", "Realized P/L"].map(
                      (h) => (
                        <th
                          key={h}
                          style={{
