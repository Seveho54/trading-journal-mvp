...
function tradeDurationDays(p: any) {
  ...
  if (!od || !cd || Number.isNaN(od.getTime()) || Number.isNaN(cd.getTime()))
    return null;
    ...
    >
      ...
      >
        ...
        <div className="p-muted" style={{ fontSize: 12 }}>
          Scale: ±{fmtMoney(maxAbs, DEFAULT_CCY)}
        </div>
      </div>
      ...
      >
        ...
        <div style={{ fontSize: 12, color: "var(--muted)" }}>
          <div style={{ marginBottom: 6 }}>
            <span style={{ color: "rgba(54,211,153,0.9)", fontWeight: 900 }}>
              {aLabel}:
            </span>{" "}
            <b style={{ color: "var(--text)" }}>{aValue}</b>
          </div>
          <div>
            <span style={{ color: "rgba(251,113,133,0.9)", fontWeight: 900 }}>
              {bLabel}:
            </span>{" "}
        ...
        <tbody>
          {(rows ?? []).slice(0, 14).map((r: any) => (
            <tr
              ...
              onClick={() =>
                router.push(`/positions?symbol=${encodeURIComponent(r.symbol)}`)
              }
            >
              <td
                style={{
...
function TickerComparePanel({
  ...
  const aList = useMemo(
    () => positions.filter((p) => normalizeSymbol(p?.symbol) === a),
    [positions, a],
  );
  const bList = useMemo(
    () => positions.filter((p) => normalizeSymbol(p?.symbol) === b),
    [positions, b],
  );
  ...
  function last3Pnls(list: any[]) {
    const sorted = [...list].sort((x, y) => {
      const tx = String(
        x?.closedAt ?? x?.timestamp ?? x?.date ?? x?.openedAt ?? "",
      );
      const ty = String(
        y?.closedAt ?? y?.timestamp ?? y?.date ?? y?.openedAt ?? "",
      );
  ...
  function MetricRow({ label, av, bv, clsA, clsB }: any) {
    return (
      <div style={rowStyle}>
        ...
        <div className={clsA} style={{ fontWeight: 900 }}>
          {av}
        </div>
        <div className={clsB} style={{ fontWeight: 900 }}>
          {bv}
        </div>
      </div>
    );
  ...
  return (
    <div style={{ gridColumn: "1 / -1" }}>
      ...
      >
        <div>
          <div className="p-muted" style={{ fontSize: 12, fontWeight: 800 }}>
            Ticker A
          </div>
          <select
            value={a}
      ...
      <MetricRow
        label="Total Net P&L"
        av={fmtMoney(aStats.net, DEFAULT_CCY)}
        bv={fmtMoney(bStats.net, DEFAULT_CCY)}
        clsA={pnlClass(aStats.net)}
        clsB={pnlClass(bStats.net)}
      />
      <MetricRow
        label="Win Rate"
        av={fmtPercent(aStats.winRate)}
        bv={fmtPercent(bStats.winRate)}
      />
      <MetricRow
        label="Average Trade Duration (days)"
        av={fmt2(aStats.avgDuration)}
        bv={fmt2(bStats.avgDuration)}
      />
      <MetricRow
        label="Largest Win"
        av={fmtMoney(aStats.largestWin, DEFAULT_CCY)}
        bv={fmtMoney(bStats.largestWin, DEFAULT_CCY)}
        clsA={pnlClass(aStats.largestWin)}
        clsB={pnlClass(bStats.largestWin)}
      />
      <MetricRow
        label="Average Win Size"
        av={fmtMoney(aStats.avgWin, DEFAULT_CCY)}
        bv={fmtMoney(bStats.avgWin, DEFAULT_CCY)}
        clsA={pnlClass(aStats.avgWin)}
        clsB={pnlClass(bStats.avgWin)}
      />
      <MetricRow
        label="Largest Loss"
        av={fmtMoney(aStats.largestLoss, DEFAULT_CCY)}
        bv={fmtMoney(bStats.largestLoss, DEFAULT_CCY)}
        clsA={pnlClass(aStats.largestLoss)}
        clsB={pnlClass(bStats.largestLoss)}
      />
      <MetricRow
        label="Average Loss Size"
        av={fmtMoney(aStats.avgLoss, DEFAULT_CCY)}
        bv={fmtMoney(bStats.avgLoss, DEFAULT_CCY)}
        clsA={pnlClass(aStats.avgLoss)}
        clsB={pnlClass(bStats.avgLoss)}
      />
      <MetricRow
      ...
      >
        <button
          className="btn-secondary"
          onClick={() =>
            router.push(`/positions?symbol=${encodeURIComponent(a)}`)
          }
        >
          Open {a}
        </button>
        <button
          className="btn-secondary"
          onClick={() =>
            router.push(`/positions?symbol=${encodeURIComponent(b)}`)
          }
        >
          Open {b}
        </button>
...
export default function PerformancePage() {
  ...
  useEffect(() => {
    const e = localStorage.getItem("tv_email");
    setEmail(e);
    setChecked(true);
  }, []);

  useEffect(() => {
    track("view_performance_page");
  }, []);
  ...
  const byMonthPos = useMemo(
    () => ((data as any)?.byMonthPositions ?? []) as any[],
    [data],
  );
  ...
  const summary = useMemo(
    () => buildTradeSummaryFromPositions(positions),
    [positions],
  );

  const monthLabels = useMemo(() => {
    const months = [...byMonthPos].sort((a: any, b: any) =>
  ...
  const dist = useMemo(() => {
    ...
    const buckets = new Map<
      string,
      { label: string; count: number; net: number }
    >();
  ...
  const risk = useMemo(() => {
    ...
    const mean = avgDay;
    const variance =
      pnls.reduce((acc, x) => acc + Math.pow(x - mean, 2), 0) /
      Math.max(1, count - 1);
  ...
  const history = useMemo(() => {
    ...
    const monthlyNet = monthDays.reduce(
      (acc: number, d: any) => acc + (d.totalNetProfit ?? 0),
      0,
    );
    ...
    const yearDays = days.filter((d: any) =>
      String(d.day ?? "").startsWith(curYearKey),
    );
    const yearlyNet = yearDays.reduce(
      (acc: number, d: any) => acc + (d.totalNetProfit ?? 0),
      0,
    );
    ...
    if (withDur.length) {
      const sorted = [...withDur].sort(
        (a: any, b: any) => (b._durDays ?? 0) - (a._durDays ?? 0),
      );
      longest = sorted[0];
      shortest = [...sorted].reverse()[0];
      avgDurDays =
        withDur.reduce((acc: number, p: any) => acc + (p._durDays ?? 0), 0) /
        withDur.length;
    }
    ...
    const bucketRows = buckets.map((b) => {
      ...
      const net = list.reduce(
        (acc: number, p: any) => acc + (p.netProfit ?? 0),
        0,
      );
      const count = list.length;
  ...
  return (
    <main>
      ...
      <div className="card" style={{ padding: 18, marginBottom: 12 }}>
        <div className="h1">Performance</div>
        <p className="p-muted">
          Session: <b>{data.uploadedFileName}</b> · Positions:{" "}
        ...
        items={[
          {
            id: "summary",
            label: "Trade Summary",
            hint: "KPIs + distribution",
            onClick: () => scrollTo(refSummary),
          },
          {
            id: "history",
            label: "Performance History",
            hint: "Equity curve + month/day",
            onClick: () => scrollTo(refHistory),
          },
          {
            id: "ticker",
            label: "Ticker Analytics",
            hint: "Symbol leaderboard",
            onClick: () => scrollTo(refTicker),
          },
          {
            id: "risk",
            label: "Risk Analytics",
            hint: "Daily stats + volatility",
            onClick: () => scrollTo(refRisk),
          },
          {
            id: "strategy",
            label: "Strategy Analytics",
            hint: "Plan / placeholder",
            onClick: () => scrollTo(refStrategy),
          },
        ]}
      />
      ...
      >
        <div style={sectionTitleStyle()}>
          ...
          <div className="p-muted">
            High-level snapshot (based on closed positions)
          </div>
        </div>
        ...
        >
          ...
          <div style={{ display: "grid", gap: 12 }}>
            <div style={cardInner()}>
              ...
              <div
                className={pnlClass(summary.totalNetProfit)}
              ...
              <div className="p-muted" style={{ fontSize: 12, marginTop: 6 }}>
                ...
                <b style={{ color: "var(--text)" }}>
                  {fmtMoney(summary.expectancy, DEFAULT_CCY)}
                </b>{" "}
                / position
              </div>
            </div>
            ...
            <div style={cardInner()}>
              ...
              <div className="p-muted" style={{ fontSize: 12, marginTop: 6 }}>
                GP:{" "}
                <span className={pnlClass(summary.grossProfit)}>
                  {fmtMoney(summary.grossProfit, DEFAULT_CCY)}
                </span>
                · GL:{" "}
                <span className={pnlClass(summary.grossLoss)}>
                  {fmtMoney(summary.grossLoss, DEFAULT_CCY)}
                </span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="p-muted" style={{ fontSize: 12, marginTop: 8 }}>
              Purpose: quick sense of how your results cluster (small wins vs
              big wins).
            </div>
          </div>
          ...
          <div style={{ display: "grid", gap: 12 }}>
            ...
            <div style={cardInner()}>
              ...
              <div
                className={pnlClass(summary.maxDrawdown)}
              ...
              <div className="p-muted" style={{ fontSize: 12, marginTop: 6 }}>
                Win streak:{" "}
                <b style={{ color: "var(--text)" }}>{summary.maxWinStreak}</b> ·
                Loss streak:{" "}
          ...
          <div style={cardInner()}>
            ...
            {summary.bestPosition ? (
              <div style={{ marginTop: 8 }}>
                ...
                <div
                  className={pnlClass(summary.bestPosition.netProfit)}
                ...
                <button
                  ...
                  style={{ marginTop: 10 }}
                  onClick={() =>
                    router.push(`/positions/${summary.bestPosition!.id}`)
                  }
                >
                  View Position
                </button>
          ...
          <div style={cardInner()}>
            ...
            {summary.worstPosition ? (
              <div style={{ marginTop: 8 }}>
                ...
                <div
                  className={pnlClass(summary.worstPosition.netProfit)}
                  style={{ fontWeight: 900, fontSize: 18 }}
                >
                ...
                <button
                  ...
                  style={{ marginTop: 10 }}
                  onClick={() =>
                    router.push(`/positions/${summary.worstPosition!.id}`)
                  }
                >
                  View Position
                </button>
      ...
      >
        <div style={sectionTitleStyle()}>
          ...
          <div className="p-muted">
            Time-based breakdown + duration impact (sheet-inspired)
          </div>
        </div>
          ...
          <div style={cardInner()}>
            ...
            <div style={{ display: "grid", gap: 8 }}>
              ...
              >
                ...
                <span
                  className={pnlClass(history.weeklyNet)}
                  style={{ fontWeight: 900 }}
                >
              ...
              >
                ...
                <span
                  className={pnlClass(history.monthlyNet)}
                  style={{ fontWeight: 900 }}
                >
              ...
              >
                ...
                <span
                  className={pnlClass(history.yearlyNet)}
                  style={{ fontWeight: 900 }}
                >
            ...
            >
              <div>
                ...
                <div style={{ display: "grid", gap: 6 }}>
                  {history.bestDays?.length ? (
                    history.bestDays.map((d: any) => (
                      <div
                        key={d.day}
                        style={{
                          display: "flex",
                      ...
                      >
                        ...
                        <span
                          className={pnlClass(d.totalNetProfit ?? 0)}
                          style={{ fontWeight: 900 }}
                        >
              ...
              <div>
                ...
                <div style={{ display: "grid", gap: 6 }}>
                  {history.worstDays?.length ? (
                    history.worstDays.map((d: any) => (
                      <div
                        key={d.day}
                        style={{
                          display: "flex",
          ...
          <div style={cardInner()}>
            ...
            >
              <div style={{ fontWeight: 900 }}>
                Performance by last 12 months
              </div>
              <div className="p-muted" style={{ fontSize: 12 }}>
                Monthly net PnL (not cumulative)
              </div>
            </div>
              ...
              <div style={{ display: "grid", gap: 8 }}>
                ...
                >
                  ...
                  <span style={{ fontWeight: 900 }}>
                    {history.longest?._durDays != null
                      ? `${history.longest._durDays.toFixed(1)}d`
                      : "–"}
                  </span>
                </div>
                ...
                >
                  ...
                  <span style={{ fontWeight: 900 }}>
                    {history.shortest?._durDays != null
                      ? `${history.shortest._durDays.toFixed(1)}d`
                      : "–"}
                  </span>
                </div>
                ...
                >
                  ...
                  <span style={{ fontWeight: 900 }}>
                    {Number.isFinite(history.avgDurDays) &&
                    history.avgDurDays > 0
                      ? `${history.avgDurDays.toFixed(1)}d`
                      : "–"}
                  </span>
                </div>
              </div>
        ...
        >
          <div style={cardInner()}>
            ...
            >
              ...
              <div className="p-muted" style={{ fontSize: 12 }}>
                Cumulative equity (monthly)
              </div>
            </div>
            <div style={{ marginTop: 10, height: 120 }}>
              <Sparkline
                values={equityCurveMonthly}
                labels={monthLabels}
                height={120}
              />
            </div>
      ...
      >
        <div style={sectionTitleStyle()}>
          ...
          <div className="p-muted">
            Bar chart + compare 2 tickers + ALL/LONG/SHORT tables
          </div>
        </div>
        ...
        >
          <div style={{ minWidth: 980, padding: 12 }}>
            ...
            >
              ...
              <TickerTableBlock
                title="LONGS"
                rows={tickerAgg.longs}
                router={router}
              />
              <TickerTableBlock
                title="SHORTS"
                rows={tickerAgg.shorts}
                router={router}
              />
            </div>
          </div>
      ...
      >
        <div style={sectionTitleStyle()}>
          ...
          <div className="p-muted">
            Daily results: stability, volatility, extremes
          </div>
        </div>
          ...
          <>
            ...
            >
              <div style={cardInner()}>
                ...
                <div style={{ fontSize: 22, fontWeight: 900, marginTop: 4 }}>
                  {fmtPercent(risk.profitableRate)}
                </div>
              ...
              <div style={cardInner()}>
                ...
                <div
                  className={pnlClass(risk.avgDay)}
                  style={{ fontSize: 22, fontWeight: 900, marginTop: 4 }}
                >
              ...
              <div style={cardInner()}>
                ...
                <div style={{ fontSize: 22, fontWeight: 900, marginTop: 4 }}>
                  {fmtMoney(risk.volatility, DEFAULT_CCY)}
                </div>
                <div className="p-muted" style={{ marginTop: 6, fontSize: 12 }}>
                  Sharpe-lite:{" "}
                  <b style={{ color: "var(--text)" }}>
                    {risk.sharpeLite.toFixed(2)}
                  </b>
                </div>
              </div>
            </div>
            ...
            >
              <div style={cardInner()}>
                ...
                <div style={{ marginTop: 6 }}>
                  <span
                    className={pnlClass(risk.best?.totalNetProfit ?? 0)}
                    style={{ fontWeight: 900 }}
                  >
                ...
                {risk.best?.day ? (
                  <button
                    ...
                    style={{ marginTop: 10 }}
                    onClick={() =>
                      router.push(`/positions?day=${risk.best.day}`)
                    }
                  >
                    View positions
                  </button>
              ...
              <div style={cardInner()}>
                ...
                <div style={{ marginTop: 6 }}>
                  <span
                    className={pnlClass(risk.worst?.totalNetProfit ?? 0)}
                    style={{ fontWeight: 900 }}
                  >
                ...
                {risk.worst?.day ? (
                  <button
                    ...
                    style={{ marginTop: 10 }}
                    onClick={() =>
                      router.push(`/positions?day=${risk.worst.day}`)
                    }
                  >
                    View positions
                  </button>
      ...
      >
        <div style={sectionTitleStyle()}>
          ...
          <div className="p-muted">
            What setups work best (MVP placeholder until data exists)
          </div>
        </div>
        ...
        {strategyTable.length === 0 ? (
          <div style={cardInner()}>
            ...
            <div className="p-muted" style={{ fontSize: 12 }}>
              MVP idea: add a “strategy” column in CSV or tagging in the app.
              Then we can rank strategies by PnL, winrate, and drawdown impact.
            </div>
            <div
              style={{
                display: "flex",
              ...
              <tbody>
                {strategyTable.slice(0, 20).map((s: any) => (
                  <tr key={s.strategy} style={{ cursor: "pointer" }}>
                    ...
                    >
                      <span
                        className={pnlClass(s.totalNetProfit ?? 0)}
                        style={{ fontWeight: 900 }}
                      >
      ...
      {checked && !email ? (
        <GateOverlay
          ...
          subtitle="Enter your email to access Performance & Calendar analytics."
          onSubmit={(e) => {
            localStorage.setItem("tv_email", e);
            setEmail(e);
          }}
        />
      ) : null}
